...
export class VideoPlayer extends Component {
  ...
  getPreviewForSecond(forSecond = 0, maximumSize, format = "base64") {
    ...
    return new Promise((resolve, reject) => {
      RNVideoTrimmer.getPreviewImageAtPosition(
        actualSource,
        forSecond,
        maximumSize,
        format,
  ...
  trim(options = {}) {
    ...
    if (
      options.hasOwnProperty("quality") &&
      !availableQualities.includes(options.quality)
    ) {
      // eslint-disable-next-line no-console
      console.warn(
        "Quality is wrong, Please use VideoPlayer.Constants.quality"
      );
    }
    const actualSource = getActualSource(this.props.source);
    return new Promise((resolve, reject) => {
  ...
  compress(_options = {}) {
    ...
    // };
    if (
      options.hasOwnProperty("bitrateMultiplier") &&
      options.bitrateMultiplier <= 0
    ) {
    ...
    if (
      options.hasOwnProperty("minimumBitrate") &&
  ...
  setCurrentTime(val) {
    UIManager.dispatchViewManagerCommand(
      RN.findNodeHandle(this.instance),
      ProcessingUI.Commands.setCurrentTimeViaManager,
      [val]
    );
  }
  ...
  render() {
    ...
    if (__DEV__) {
      const isCompatible = Object.values(
        VideoPlayer.Constants.resizeMode
      ).includes(resizeMode);
      if (!isCompatible) {
        console.warn(
          "Wrong resizeMode property, please use VideoPlayer.Constants.resizeMode constants"
        );
      }
    }
    ...
    return (
      <RNVideoPlayer
        ref={(component) => (this.instance = component)}
        source={actualSource}
        play={play}
        replay={replay}
        rotate={rotate}
...
const RNVideoPlayer = requireNativeComponent(
